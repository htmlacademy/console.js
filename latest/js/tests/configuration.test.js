(function () {
	...
	const checkEnumContainsValue = (enumerable, value) => {
	  return !!Object.entries(enumerable).find((entry) => entry[1] === value);
	};

	const escapeHTML = (htmlMarkup) => {
	  return htmlMarkup
	      .replace(/&/g, `&amp;`)
	      .replace(/</g, `&lt;`)
	      .replace(/>/g, `&gt;`)
...
${this._fnType === FnType.ARROW ? ` => ` : ` `}${bodyLines.join(`\n`)}`;
	...
	class Console {
	  ...
	  constructor(container, params = {}) {
	    ...
	    this.params = {
	      object: this._parseViewParams(ViewType.OBJECT, lodash_mergewith({}, params.common, params.object, customizer)),
	      array: this._parseViewParams(ViewType.ARRAY, lodash_mergewith({}, params.common, params.array, customizer)),
	      function: this._parseViewParams(ViewType.FUNCTION, lodash_mergewith({}, params.common, params.function, customizer))
	    };
