(function () {
  ...
  var CSS_URL = "//htmlacademy.github.io/console.js/1.0.2/css/style.min.css";
  var messages = [];

  var collectErr = function collectErr(evt) {
    messages.push({
      mode: Mode.ERROR,
      args: [evt.error ? evt.error : evt.message]
    });
  };
  ...
  window.console.info = collectMessages.bind({
    mode: Mode.LOG
  });
  window.console.log = collectMessages.bind({
    mode: Mode.LOG
  });
  window.console.logHTML = collectMessages.bind({
    mode: Mode.LOG_HTML
  });
  window.console.dir = collectMessages.bind({
    mode: Mode.DIR
  });
  window.console.warn = collectMessages.bind({
    mode: Mode.ERROR
  });
  window.console.error = collectMessages.bind({
    mode: Mode.ERROR
  });
  var container = null;
  ...
  var init = function init() {
    ...
    var jsConsole = new Console(container);
  ...
  var processMessages = function processMessages() {
    messages.forEach(function (_ref) {
      var _window$jsConsole, _window$jsConsole2, _window$jsConsole3, _window$jsConsole4;
      ...
      switch (mode) {
        case Mode.LOG:
          (_window$jsConsole = window.jsConsole).log.apply(_window$jsConsole, _toConsumableArray(args));

          break;

        case Mode.DIR:
          (_window$jsConsole2 = window.jsConsole).dir.apply(_window$jsConsole2, _toConsumableArray(args));

          break;

        case Mode.LOG_HTML:
          (_window$jsConsole3 = window.jsConsole).logHTML.apply(_window$jsConsole3, _toConsumableArray(args));

          break;

        case Mode.ERROR:
          (_window$jsConsole4 = window.jsConsole).error.apply(_window$jsConsole4, _toConsumableArray(args));
